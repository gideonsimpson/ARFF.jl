...
"""
    train_rwm!(F::ScalarFeaturesModel{TC,TR,TW}, data::ScalarDataSet{TC,TR,TW}, Σ::TM, options::ARFFOptions; show_progress=true, record_loss=true) where {TC<:Complex,TR<:AbstractFloat,TW<:AbstractArray{TR},TM<:AbstractMatrix}
...
* `record_loss=true` - Evaluate the specified loss function at each epoch and record
"""
function train_rwm!(F::ScalarFeatureModel{TC,TR,TW,TI,TF}, data::ScalarDataSet{TC,TR,TW}, Σ::TM, options::ARFFOptions; show_progress=true, record_loss=true) where {TC<:Complex,TR<:AbstractFloat,TW<:AbstractArray{TR},TM<:AbstractMatrix,TI<:Integer,TF<:Function}
...
"""
    train_rwm!(F::ScalarFourierModel{TB,TR,TW}, data::DataSet{TB,TR,TW}, batch_size::TI, Σ::TM, options::ARFFOptions; show_progress=true, record_loss=true) where {TB<:Complex,TR<:AbstractFloat,TW<:AbstractArray{TR},TM<:AbstractMatrix,TI<:Integer}
...
"""
    train_rwm!(F::ScalarFeatureModelModel{TB,TR,TW}, data::DataSet{TB,TR,TW}, batch_size::TI, Σ::TM, options::ARFFOptions; show_progress=true, record_loss=true) where {TB<:Complex,TR<:AbstractFloat,TW<:AbstractArray{TR},TM<:AbstractMatrix,TI<:Integer}
...
* `record_loss=true` - Evaluate the specified loss function at each epoch and record
"""
function train_rwm!(F::ScalarFeatureModel{TC,TR,TW,TI,TF}, data::ScalarDataSet{TC,TR,TW}, batch_size::TI, Σ::TM, options::ARFFOptions; show_progress=true, record_loss=true) where {TC<:Complex,TR<:AbstractFloat,TW<:AbstractArray{TR},TM<:AbstractMatrix,TI<:Integer,TF<:Function}
...
Train the Fourier feature model using a random walk Metropolis exploration
strategy with batched data, which is cycled through from epoch to epoch. Returns
the entire trajectory of models during training.

### Fields
* `F₀` - The initial state of the `FourierModel` to be trained
* `batched_data`- A vector of `DataSet` training data sets, for the purpose of
  minibatching.  These are presumed to all be the same size.
* `Σ` - Initial covariance matrix for RWM proposals
...
Train the Fourier feature model using a random walk Metropolis exploration
strategy with minibatching, randomly subsampling at each epoch. Returns the
entire trajectory of models during training.
