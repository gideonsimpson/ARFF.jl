...
function train_rwm!(F::ScalarFourierModel{TR,TB,TI,TA}, data::ScalarDataSet{TR,TB,TI}, batch_size::TI, Σ::Matrix{TR}, options::ARFFOptions; show_progress=true, record_loss=true) where {TB<:Number,TR<:AbstractFloat,TI<:Integer,TA<:ActivationFunction{TB}}
    Σ_mean, acceptance_rate, loss = train_rwm!(F, Iterators.cycle([data]), length(data), batch_size, Σ, options; show_progress=show_progress,record_loss=record_loss)
    return Σ_mean, acceptance_rate, loss
end
...
function train_rwm!(F::ScalarFourierModel{TR,TB,TI,TA}, data_sets, Σ::Matrix{TR}, options::ARFFOptions; show_progress=true, record_loss=true) where {TB<:Number,TR<:AbstractFloat,TI<:Integer,TA<:ActivationFunction{TB}}
    Σ_mean, acceptance_rate, loss = train_rwm!(F, data_sets, length(first(data_sets)), length(first(data_sets)), Σ, options; show_progress=show_progress, record_loss=record_loss)
    return Σ_mean, acceptance_rate, loss
end
...
# * `record_loss=true` - Evaluate the specified loss function at each epoch and record
# """
# function train_rwm!(F::ScalarFourierModel{TR,TB,TI,TA}, batched_data::Vector{ScalarDataSet{TR,TB,TI}}, Σ::Matrix{TR}, options::ARFFOptions; show_progress=true) where {TB<:Number,TR<:AbstractFloat,TI<:Integer,TA<:ActivationFunction{TB}}
